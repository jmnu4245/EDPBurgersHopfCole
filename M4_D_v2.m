function [x, incr, I, iter] = M4_D_v2(U, k, h, E, tol, maxiter)
    ...
    while incr > tol && iter < maxiter
        ...
        T_gamma = - divdiff_analitica( x, x0,A,B,U_jj(1), U_jj(end));
        
        % w = x - Gamma * F(x)
        w = x + T_gamma \ Fx;
        
        % [w, x; F]
        T_wx = divdiff_analitica( w, x,A,B,U_jj(1), U_jj(end));
        
        % y = x - [w, x; F]^-1 * F(x)
        y = x - T_wx \ Fx;
        
        % mu = I - [w, x; F]^-1 * [y, w; F]
        T_yw = divdiff_analitica(y, w,A,B,U_jj(1), U_jj(end));
        mu = Id - T_wx \ T_yw;
        ...
        % z = [y, x; F]^-1 * F(y)
        T_yx = divdiff_analitica(y, x,A,B,U_jj(1), U_jj(end));
        Fy = Func(y);
        z = T_yx \ Fy;
