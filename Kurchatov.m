function [x, incr, I, iter] = Kurchatov(U, k, h, E, tol, maxiter)
    ...
    while incr > tol && iter < maxiter
        ...
        % Gamma = - [2x(k)-x(k-1), x(k-1); F]^-1. Se calcula solo -[2x(k)-x(k-1), x(k-1); F] y luego
        % la inversa se calcula con la contrabarra.
        T_gamma = - divdiff(Func, 2*x-x0, x0);
        ...
        % x = y - H(mu) * z
        x = y - H_val * z;
        incr = norm(x - x0, inf) + norm(Fy, inf);
        iter = iter + 1;
    end
end